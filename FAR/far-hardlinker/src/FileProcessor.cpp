...
void scan_single_folder(fsys::Node_t folder)
{
	...
	for (auto it = dir.begin(); it != dir.end(); ++it) {
		...
		if (it->is_dir()) {
			...
			global::vars().files.emplace_back(simstd::make_shared<fsys::File>(*it, folder));
		}
	}
}
...
//	return 0;
	...
	while (!vars().folders.empty())
	{
		auto folder = vars().folders.back();
		vars().folders.pop_back();
