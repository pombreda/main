...
//				traits_type::construct(allocator, addressof(*first));
//			return first;
//		}
//
...
//			for (; first != last; ++first)
//				traits_type::construct(allocator, addressof(*first), value);
//			return first;
//		}
		...
		{
			::new (static_cast<void*>(ptr), nothrow) Type(forward<Args>(args)...);
		}
		...
		void destroy(Allocator& allocator, ForwardIterator first, ForwardIterator last)
		{
			using traits_type = allocator_traits<Allocator>;
			for (; first != last; ++first)
				traits_type::destroy(allocator, addressof(*first));
		}
...
//		_Destroy_aux<__has_trivial_destructor(Value_type)>::destroy(first, last);
//	}
//
...
//	{
//		for (; first != last; ++first)
//			alloc.destroy(_addressof(*first));
//	}
//
...
//	}
	...
	ForwardIt uninitialized_copy(InputIt first, InputIt last, ForwardIt d_first)
	{
		for (; first != last; ++first, ++d_first) {
			pvt::construct(addressof(*d_first), *first);
		}
		return d_first;
	}
	...
	{
		using alloc_traits_type = allocator_traits<Allocator>;
		for (; first != last; ++first, ++d_first) {
			alloc_traits_type::construct(allocator, addressof(*d_first), *first);
		}
		return d_first;
	}
...
//	{
//		return uninitialized_copy_a(make_move_iterator(first), make_move_iterator(last), d_first, allocator);
//	}
	...
	ForwardIt uninitialized_default_a(ForwardIt first, ForwardIt last, Allocator& allocator)
	{
		using alloc_traits_type = allocator_traits<Allocator>;
		for (; first != last; ++first) {
			alloc_traits_type::construct(allocator, addressof(*first));
		}
		return first;
	}
	...
	{
		for (; first != last; ++first) {
			pvt::construct(addressof(*first), val);
		}
		return first;
	}
	...
	ForwardIt uninitialized_fill_a(ForwardIt first, ForwardIt last, const Type& val, Allocator& allocator)
	{
		using alloc_traits_type = allocator_traits<Allocator>;
		for (; first != last; ++first) {
			alloc_traits_type::construct(allocator, addressof(*first), val);
		}
		return first;
	}
